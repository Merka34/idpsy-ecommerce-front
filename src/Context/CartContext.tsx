...
export const CartContextProvider = ({ children }) => {
    ...
    const saveLocalCart = (cartItems: any) => {
        try {
        ...
        if (isAuthenticated()) {
            ...
            try {
                ...
                if (existingIndex > -1) {
                    ...
                    (currentCart[existingIndex] as any).quantity += quantity
                } else {
                    // Nuevo producto: agregarlo
                    currentCart.push({ ...product, quantity } as never)
                }
    ...
    const updateQuantity = async (productId: string, newQuantity: number) => {
        ...
        if (isAuthenticated()) {
            ...
            // Usuario no autenticado: usar localStorage
            try {
                const currentCart = cart.map((item: any) =>
                    item._id === productId
                        ? { ...item, quantity: newQuantity }
                        : item
                )
                setCart(currentCart as never)
                saveLocalCart(currentCart)
                toast.success('Cantidad actualizada')
            } catch (error) {
